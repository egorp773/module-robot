...
# === Фаза 1: 1.5м прямо ===
Write-Host ""
Write-Host "[*] Phase 1: 1.5m straight (M,70,70) - timeout 60s, cross-track guard 0.5m"
$startTime = Get-Date
$lastDist = 0
$doneP1 = $false
while (-not $doneP1) {
...
Write-Host ""
Write-Host "[*] Phase 2: 90 deg right turn in place (M,-40,40) - timeout 30s"
$startTime = Get-Date
$doneP2 = $false
while (-not $doneP2) {
...
# === Фаза 3: 1м прямо ===
Write-Host ""
Write-Host "[*] Phase 3: 1.0m straight from P2 (M,70,70) - timeout 30s, cross-track 0.5m"
$startTime = Get-Date
$lastDist = 0
$doneP3 = $false
while (-not $doneP3) {
